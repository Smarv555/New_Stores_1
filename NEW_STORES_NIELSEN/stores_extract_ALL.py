...
def stores_extract(country, txt_name, excel_name):
    ...
    # Storing sheet information in lists
    sheets_list, sheet_names_list = open_xl(excel)

    with open(notepad, 'w') as file:

        # Sheets cycle
        for sheet in sheets_list:
            sheet_name = sheet_names_list[sheets_list.index(sheet)]
            ...
            for row in range(4, 9999):
                ...
                # Columns extraction order\ IVROC status check
                if sheet_name.upper() == 'SCANNING' and str(sheet.cell(row=row, column=15).value).lower() != 'done':
                    file.write(
                ...
                elif sheet_name.upper() == 'AUDIT' and str(sheet.cell(row=row, column=12).value).lower() != 'done':
                    file.write(
                        f'{ac_nshopid["AUDIT"]}\t'
                        f'{ac_shopstatus}\t'
                ...
                elif sheet_name.upper() == 'A2S' and str(sheet.cell(row=row, column=15).value).lower() != 'done':
                    file.write(
                        f'{ac_nshopid["A2S"][0]}\t'
                        f'{ac_shopstatus}\t'
...
def storedtgroups_extract(country, txt_name, excel_name):
...
def storexcodegroups_extract(country, txt_name, excel_name):
...
# AUDIT LHHT!
def lbatchstores_extract(country, txt_name, excel_name):
...
def store_group_stores(country, txt_name, excel_name):
...
def lbatches_extract(country, txt_name, excel_name):
    ...
    with open(notepad, 'w') as file:
        ...
        for sheet in sheets_list:
            ...
            for row in range(4, 9999):
                ...
                # Columns extraction order
                if sheet_name.upper() == 'SCANNING' and str(sheet.cell(row=row, column=15).value).lower() != 'done' and new_batch:
                    file.write(
                        f'{ac_lbatchid["SCANNING"]}\t'
                        f'{nc_activeflag}\n'
                    )
                elif sheet_name.upper() == 'AUDIT' and str(sheet.cell(row=row, column=12).value).lower() != 'done' and new_batch:
                    file.write(
                        f'{ac_lbatchid["AUDIT"]}\t'
                        f'{nc_activeflag}\n'
                    )
                elif sheet_name.upper() == 'A2S' and str(sheet.cell(row=row, column=15).value).lower() != 'done' and new_batch:
                    file.write(
                        f'{ac_lbatchid["A2S"]}\t'
                        f'{nc_activeflag}\n'
                    )
...
def lbatchdtgroups_extract(country, txt_name, excel_name):
...
if __name__ == '__main__':
    try:
        ...
        # Spell check
        if spell_check(country_input, txt_path, spell_check_txt, excel_name_input):
            # Stores extraction
            stores_extract(country_input, stores_txt, excel_name_input)
            # Storedtgroups extraction
            storedtgroups_extract(country_input, dtgroups_txt, excel_name_input)
            # Storexcodegroups extraction
            storexcodegroups_extract(country_input, xcodegroups_txt, excel_name_input)
            # Lbatchstores extraction
            lbatchstores_extract(country_input, lbatchstores_txt, excel_name_input)
            # Store_group_stores extraction
            store_group_stores(country_input, store_group_stores_txt, excel_name_input)
            # Lbatches extraction
            lbatches_extract(country_input, lbatches_txt, excel_name_input)
            # Lbatchdtgroups extraction
            lbatchdtgroups_extract(country_input, lbatchdtgroups_txt, excel_name_input)
